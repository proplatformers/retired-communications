...
/**
 * This class, when instantiated, is used to handle one and only one client connection to
 * the CSTAServer.  The CSTAServer can handle multiple client conntions by maintaining
 * a List of ServeOneClient objects.
 * The opposite of this class on the client side is TCPClient.
 *
 */
public class ServeOneClient extends Thread {
     ...
     *
     *
     * @param s the socket in this tcp/ip connection
     * @param server the CSTAServer that handles this ServeOneClient object
     * @throws IOException the exception thrown if the input/output streams aren't set up properly
     */
    ...
    /**
     * encodes a string to send out back to the client by replacing all
     * DLE characters with DLE DLE sets and wraps it in the communications
     * protocol
     *
     *
     * @return the encoded string
     * @param curOutStr the string to encode
     */
    ...
    /**
     * This method is called by the CSTAServer and sends the string back to the client which
     * is on the other end of this tcp/ip connection.
     *
     *
    ...
    public void SendToClient(StringBuffer str){
        ...
        alog.info(this.getClass().getName() + " ---> " + "SERVER TO CLIENT - R: " + tmp) ;
        try{
    ...
    /**
     * decodes a string to send out back to the client by replacing all
     * DLE DLE sets with DLE characters and strips off what is left from the
     * communications protocol
     *
     *
     * @param currentByte the current byte in the transmission to be added to the buffer
     */
    private void AddToBuffer( int currentByte ){
        if(!complete){
            if ( !CheckReceived(currentByte) )
                ...
                complete = true ;//this is complete, just for the trailing character not to be added.
        } else ;
    }
     ...
     * @param currentByte the current byte to be checked
     */
     ...
     * @param curInStr the string to decode
     */
    ...
    /**
     * receives data from the tcp/ip socket and adds it to the buffer until reception is
     * complete.  The data is then decoded and sent to the CSTAServer for processing
     *
     */
    public void run(){
            ...
            while(this.isRunning()){
                ...
                if( bytes > 0 ){
                    ...
                    if(complete == true){
                        ...
                        //**LOG**RXTX FINE = Network communications - completed message received <string> - OK
